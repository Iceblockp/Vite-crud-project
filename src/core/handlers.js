...
export const editCellHandler = (event) => {
    if (event.target.classList.contains("cell-editable")) {
        ...
        const currentCellColumName = currentCell.getAttribute("cell-col")
        ...
        input.addEventListener("blur", () => {
            ...
            const updateData = {};
            updateData[currentCellColumName] = newValue;
            ...
            }).then(res => res.json()).then(json => toast("Update successfully"))
...
export const searchInputHandler = (event) => {
    ...
    fetch(url("/courses?title[like]=" + event.target.value)).then(res => res.json()).then(json => {
       ...
       if(json.length){
        ...
        rowGroup.innerHTML = `<tr><td colspan='5' class='text-center px-6 py-4'>There is no Course.
        <a class='underline' href='http://${location.host}' >Browse all</a></td></tr>`
       }
    });
}